import { g as renderElement, s as spreadAttributes, u as unescapeHTML, r as renderTemplate, h as removeBase, i as isRemotePath, j as unflatten, A as AstroError, U as UnknownContentCollectionError, o as object, k as date, l as array, R as RenderUndefinedEntryError, n as escape, p as string, q as prependForwardSlash, t as createHeadAndContent, d as renderComponent } from './entrypoint_DqZWjQNj.mjs';
...
class ImmutableDataStore {
  ...
  static async fromModule() {
    try {
      const data = await import('./_astro_data-layer-content_BlPUYyid.mjs');
...
function createGetCollection({
  ...
  return async function getCollection(collection, filter) {
    ...
    if (store.hasCollection(collection)) {
      const { default: imageAssetMap } = await import('./content-assets_DBi2gn8K.mjs');
      const result = [];
...
async function updateImageReferencesInBody(html, fileName) {
  ...
  const { getImage } = await import('./_astro_assets_Cn5DiJxa.mjs').then(n => n._);
...
async function renderEntry(entry) {
  ...
  if (entry.deferredRender) {
    try {
      const { default: contentModules } = await import('./content-modules_MjoGJ5oY.mjs');
